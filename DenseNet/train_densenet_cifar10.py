...
class TestCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        self.model.save_weights('./model_cifar10/model1.h5')

        with open('./model_cifar10/start_epoch', 'wb') as f:
            cPickle.dump(epoch, f, True)

        with open('./model_cifar10/current_lr', 'wb') as f:
...
if not isFirstTimeTrain:
    model1.load_weights('./model_cifar10/model1.h5')

    with open('./model_cifar10/start_epoch') as f:
        start_epoch = cPickle.load(f)

    with open('./model_cifar10/current_lr') as f:
    ...
    model1.save_weights('./model_cifar10/model1.h5')
...
results = model1.fit(x=X_train,
                     ...
                     callbacks=[TestCallback((X_test, y_test)),
                                LrDecay(init_lr, int(n_epochs * 0.5), int(n_epochs * 0.75))])
